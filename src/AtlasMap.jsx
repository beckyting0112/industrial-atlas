...
export default function AtlasMap({ assets, flows, newsEvents=[], chokepoints=[], selectedChokepoint, onChokepointSelect=()=>{}, supplyLinks=[], companyHubs=[], countryProfiles=[], selected, onSelect, metric="global_iron_ore_production_share", industry="iron-ore", focus }) {
  ...
  return <div className="svg-atlas-shell">
    ...
    >
      ...
      <g className="atlas-world" transform={`translate(${view.x} ${view.y}) scale(${view.scale})`}>
        ...
        <g className="atlas-news-events">{newsEvents.map(item=>{const point=projection([Number(item.longitude),Number(item.latitude)]);if(!point)return null;return <g className="news-event-marker" key={item.id} transform={`translate(${point[0]} ${point[1]})`} onPointerMove={event=>{event.stopPropagation();setHovered(null);setHoveredRoute(null);setHoveredNews({item,x:Math.min(event.clientX+14,window.innerWidth-330),y:Math.min(event.clientY+14,window.innerHeight-230)})}} onPointerLeave={()=>setHoveredNews(null)}><circle className="event-hit" r={12/Math.sqrt(view.scale)}/><circle className="event-pulse" r={9/Math.sqrt(view.scale)}/><circle className={`event-core severity-${item.severity}`} r={4/Math.sqrt(view.scale)}/></g>})}</g>
    ...
    {hoveredNews&&<div className="news-event-hover" style={{left:hoveredNews.x,top:hoveredNews.y}}><span>WIP · TEMPORARY EVENT</span><h3>{hoveredNews.item.headline}</h3><div><b>{hoveredNews.item.publisher}</b><time>{new Date(hoveredNews.item.published_at).toLocaleString(undefined,{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}</time></div><p>{hoveredNews.item.location_label||hoveredNews.item.route_tags||"Location under review"}</p><small>{hoveredNews.item.review_status==="candidate"?"Auto-triaged · analyst review required":hoveredNews.item.review_status} · expires from map after 72 hours</small></div>}
