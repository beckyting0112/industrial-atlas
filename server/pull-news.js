...
for(const query of queries){
  const gdeltParams=new URLSearchParams({query,mode:"artlist",format:"json",maxrecords:"100",timespan:"36h",sort:"datedesc"});
  try{const payload=await fetchJson(`https://api.gdeltproject.org/api/v2/doc/doc?${gdeltParams}`);results.push(...(payload.articles||[]))}catch(error){console.error(`GDELT unavailable: ${error.message}`)}
  const rssParams=new URLSearchParams({q:`${query} when:2d`,hl:"en-US",gl:"US",ceid:"US:en"});
  try{results.push(...parseGoogleNews(await fetchText(`https://news.google.com/rss/search?${rssParams}`)))}catch(error){console.error(`Google News fallback unavailable: ${error.message}`)}
}
...
for(const a of articles){
  ...
  insert.run({id,cluster_key,headline:a.title,publisher:a.domain||a.sourcecountry||"Publisher",source_url:a.url,published_at:published,collected_at:now.toISOString(),event_type:type,severity:score>=85?"critical":score>=70?"high":"medium",materiality_score:score,commodity_tags:tags.join(", "),country_tags:loc?.[4]||"",route_tags:loc?.[5]||"",source_tier:tier,source_rationale:tier===1?"Primary institution or top-tier financial wire/publication":tier===2?"Specialist or established general-news source":"Discovery source; corroboration required",latitude:loc?.[3]??null,longitude:loc?.[2]??null,location_label:loc?.[1]||null,reported_summary:"A meaningful factual summary is pending source review.",summary_provenance:"summary pending review",observed_impact:impacts[type],analyst_inference:inferences[type],monitor_next:"Confirm with a primary source or second credible report; monitor throughput, vessel calls, prices, freight and inventories as applicable.",map_expires_at:loc?expires:null});stored++;
}
...
db.close();
if(!stored){console.error("No relevant candidates were stored; failing the refresh so the automation cannot report a false success.");process.exitCode=2}
